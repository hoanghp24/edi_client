  ...
  const userMenuItems: MenuProps['items'] = [
    {
      ...
      label: (
        <div
          style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '4px 0', cursor: 'default', maxWidth: 280 }}
        >
          <Avatar size={44} icon={<User size={26} />} style={{ backgroundColor: '#e30613', flexShrink: 0 }} />
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <Text strong style={{ fontSize: 15, lineHeight: 1.2 }} ellipsis={{ tooltip: displayName }}>
              {displayName}
            </Text>
            <Text style={{ fontSize: 12, marginTop: 4 }} ellipsis={{ tooltip: displayEmail }}>
              {displayEmail}
            </Text>
          </div>
        </div>
    ...
    >
      ...
      >
        <Avatar
          size={34}
          icon={<User size={20} />}
          style={{ backgroundColor: '#e30613', color: '#fff', boxShadow: '0 2px 6px rgba(227, 6, 19, 0.25)' }}
        />
